...
void *malloc(size_t size) {
    if (size < BLOCK_LENGTH_SMALL_MIN)
        return ft_malloc_in_specific_zone_type(size, &g_.tiny_first_zone, &g_.tiny_last_zone, ZONE_TOTAL_SIZE_TINY);
    else if (size < BLOCK_LENGTH_LARGE_MIN)
        return ft_malloc_in_specific_zone_type(size, &g_.small_first_zone, &g_.small_last_zone, ZONE_TOTAL_SIZE_SMALL);
